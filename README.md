...
| `@textfilters/profanity` | [`packages/profanity`](packages/profanity) | Dictionary-backed profanity filtering, obfuscation handling, taxonomy metadata, and validation. |
| `@textfilters/spam` | [`packages/spam`](packages/spam) | Stateful interval, duplicate, burst, and actor-based anti-spam checks. |

Each workspace owns its runtime implementation, tests, package README, public
API examples, and independently versioned changelog. The repository root owns
shared development, CI, release automation, compatibility policy, and
benchmarks.
...
The publish job accepts only released paths from the six-package allowlist and
