...
node --input-type=module --eval "import { readFileSync } from 'node:fs'; const lock = JSON.parse(readFileSync('package-lock.json', 'utf8')); const versions = Object.entries(lock.packages).filter(([path, pkg]) => path.endsWith('node_modules/@textfilters/core') && pkg.version).map(([, pkg]) => pkg.version); if (versions.length !== 1 || versions[0] !== '0.3.0') throw new Error('Expected exactly one @textfilters/core 0.3.0, got ' + (versions.join(', ') || 'none'));"
