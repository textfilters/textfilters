...
const PLAIN_WORD_LABEL_RE = /^\p{L}+$/u;
...
const hasPlainLabelBoundaries = (
  ...
  for (let cursor = dot.end; cursor < label.start; cursor++) {
    if (meta.whitespace[cursor]) {
      sawWhitespace = true;
      continue;
    }
    if (isIgnorableFormatting(meta, cursor) && !sawWhitespace) continue;
...
const isSentenceWordLabel = (
  ...
  if (!PLAIN_WORD_LABEL_RE.test(source)) return false;
    ...
    if (continuationDot && !independentSuffix) {
      ...
      if (continuedDomain && continuedDomain.end > domain.end) {
        ...
        const preferredContinuation = sentenceContinuation
          ...
          : null;
        if (preferredContinuation === continuedDomain && domainIsAllowed) {
